...
// 必要なbinのリストナンバーを見つける -> とりあえず1000ごとに区切ってみる
int find_binnum(size_t size){
    for (int i=0;i<6;i++){
        // サイズがここに収まってたらbinサイズを返して出力
        if (size >= i*1000 && size<(i+1)*1000){
            return i;

        }
    //最後まで行っても見つからなかったら一番最後のbinを出力する
    }
    return 5;
}
...
void my_add_to_free_list(my_metadata_t *metadata) {
  ...
  int bin = find_binnum(metadata->size);
